// Configuración de la API y selectores
const API_URL = "https://vj.interfaces.jima.com.ar/api/v2"

const carouselContainer = document.querySelector(".destacados-carousel-inner") 
...
function showSlide(index) {
  ...
  const slides = document.querySelectorAll(".destacados-card-content") 
...
async function loadCarouselData() {
  ...
  if (!carouselContainer) {
    console.error("El elemento .destacados-carousel-inner no fue encontrado.")
    return
  }
  ...
  try {
    ...
    games.forEach((game, index) => {
      ...
      const slideHtml = `
                <div class="destacados-card-content ${isActive}" tabindex="0" role="group" aria-label="Slide ${index + 1} de ${games.length}">
                    <div class="destacados-image-section">
                        <img src="${game.background_image_low_res}" alt="${game.name}" class="destacados-game-image">
                    </div>
                    <div class="destacados-text-section">
                        <h1 class="destacados-card-title">${game.name}</h1>
                        <p class="destacados-card-description">
                            ${truncatedDescription} 
                        </p>
                        <button class="destacados-play-button">Jugar</button>
                    </div>
                </div>
            `
